...
Below I propose an effective way (sort of a hack) that works for me. Instead we will symlink all the ROOT sources into the project tree, but not include them in the build. This way source navigation and debug source lookup will work just fine without building ROOT from source. Steps are following:
